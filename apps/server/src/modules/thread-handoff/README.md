...
Thread Handoff owns explicit provider-to-provider transfer semantics and provenance. It is disabled by default behind the App feature flag `threadHandoffs`. When enabled, it validates that the source is idle, creates a destination Session on another compatible enabled provider target, imports completed top-level user and assistant messages through Chat Runtime's `MessageImported` event path, and compensates by deleting the destination if bootstrap import fails.

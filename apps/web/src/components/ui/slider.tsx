    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={`thumb-${index}`}
          className="relative block size-3.5 shrink-0 rounded-[4px] border border-input bg-background ring-ring/40 transition-[color,box-shadow] select-none after:absolute after:-inset-2 focus-visible:ring-2 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )

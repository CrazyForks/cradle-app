...
The snapshot chunk is a recovery control frame and is never persisted as a `UIMessage` part. Active in-memory state is the recovery authority for reconnects and late subscribers. Server `run_stream_checkpoints` remain the separate crash-recovery authority after the process that owned the live state is gone.
